  ...
  monaco.languages.setMonarchTokensProvider('seesaawiki', {
    ...
    tokenizer: {
      root: [
        ...
        { include: '@inline' },
        [/^\[(?:\+|-)\]/, { token: 'keyword.control', bracket: '@open' }],
        [/^\[END\]/, { token: 'keyword.control', bracket: '@close' }],
        [/^(#)(contents)(\()(1|2)(\))/, [
          'keyword.control',
          'keyword',
      ...
      inline: [
        [/(&|#)(ref|attachref)(\()/, [
          'keyword.control',
          'keyword',
          { token: 'delimiter.parenthesis', bracket: '@open', next: '@ref' },
        ]],
        [/(&|#)(video|audio)(\()(.*?)(\))/, [
          'keyword.control',
          'keyword',
      ...
      links: [
        [/>{1,3}/, { token: 'delimiter.angle', switchTo: '@linkTarget' }],
        { include: '@inline' },
        { include: '@linkTarget' },
      ],
      linkTarget: [
      ...
      color: [
        [/#[0-9A-Fa-f]{8}|#[0-9A-Fa-f]{6}|#[0-9A-Fa-f]{3}|[a-zA-Z]+/, 'constant.other.colorcode'],
        [/\)/, { token: 'delimiter.parenthesis', bracket: '@close' }],
        [/\{/, { token: 'delimiter.curly', bracket: '@open', switchTo: '@rootCloseCurlyBracket' }],
        [/,/, 'delimiter'],
        [/\s+/, ''],
      ],
      table: [
        // A row being edited may have no closing pipe; restart on the next line.
        ...
        [/(r)(\[)/, [
          'keyword.control',
          { token: 'keyword.control', next: '@tableRowConfig' },
        ]],
        [/!|~|>|\^/, 'keyword.parameter'],
        { include: '@root' },
      ],
