...
- **リアルタイムプレビュー**
  - 右側に編集中のページのプレビューをリアルタイムで表示します。標準のプレビュー機能と異なり、オフラインでレンダリングします(ただし、リンク先ページの存在確認のみ通信を伴います)。
- **コードの折りたたみ(Folding Range)**
  - 見出しや`[+]`/`[-]`〜`[END]`ブロックを折りたたみ可能にします。
- **エラー表示**
  - 以下の記法エラーを検出し、エディター上およびアウトライン下のPROBLEMSパネルに一覧表示します。
    - `[+]`/`[-]`に対応する`[END]`が見つからない、または対応する開始タグのない`[END]`がある
    - 表の行が `|` または `|c` で終わっていない
    - `{|...|}` で囲まれた表の途中に空行がある
- スマートペースト機能
...
- その他
  - 画像URLホバーでプレビュー表示
  - 文字のエスケープ/アンエスケープ機能
    - 文字を選択した状態で右クリックメニューから実行するか、Command Paletteから`Escape`または`Unescape`を検索して実行
  - リンクを`Ctrl+クリック`でリンクを開く、アンカーにジャンプ
  - 差分画面で変更点へジャンプ
  - 編集時のエディターのサイズを大きくする
  - …など
...
|------------|------|--------|
| `link` | リンク | `[[リンク先]]` |
| `link (with text)` | テキスト付きリンク | `[[表示テキスト>リンク先]]` |
...
- `/`を含むページ名の参照をCtrl+クリックで開けない
- ショートカットやスニペットの前後で`pushUndoStop`されてない
